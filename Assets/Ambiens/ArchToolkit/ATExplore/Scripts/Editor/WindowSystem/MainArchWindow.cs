...
using ArchToolkit.Editor.Setup;
#if UNITY_EDITOR
using UnityEditor;
    ...
    public class MainArchWindow : MainArchWindowBase
    {
        ...
        public static void Init()
        {
            ...
            if(!AssetDatabase.IsValidFolder("Assets/TextMesh Pro")){
                Debug.Log("<color=red>AT+Explore</color> Text Mesh Pro Extra stuff not installed, please import everything from the next window!");
...
#if !AT_EXPLORE_INIT
            if(!this.isCheckingDependencies && CurrentCheckingDependency==0)
            {
                ...
                Debug.Log("<color=red>AT+Explore</color> All dependencies installed, almost ready!");

                InputAndTagSetup.AddScriptingSymbol();
            }
        ...
        private void ApplyDependencies()
        {
             try{
                ...
                Debug.Log("<color=red>AT+Explore</color> Apply Dependencies => " + (CurrentCheckingDependency+1) + "/" + this.dependencies.Count+" "+this.dependencies[CurrentCheckingDependency]);
        ...
        void DependenciesProgress()
        {
            if (this.addRequest.IsCompleted)
            {
                if (this.addRequest.Status == StatusCode.Success)
                {
                    ...
                    Debug.Log("<color=red>AT+Explore</color> Requesting the next package");
                    ...
                    Debug.Log("<color=red>AT+Explore</color> Failed To Install: " + this.addRequest.Result.packageId);
