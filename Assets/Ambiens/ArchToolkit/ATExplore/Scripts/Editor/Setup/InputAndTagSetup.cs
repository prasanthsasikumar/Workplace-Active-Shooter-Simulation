...
namespace ArchToolkit.Editor.Setup
{
...
#endif
    {
        ...
        {
            Debug.Log("<color=red>AT+Explore</color> Adding necessary Scripting Symbols...");
            string definesString = PlayerSettings.GetScriptingDefineSymbolsForGroup(EditorUserBuildSettings.selectedBuildTargetGroup);
            List<string> allDefines = definesString.Split(';').ToList();
        ...
        public void OnPackageAddedOrUpdated(UnityEditor.PackageManager.PackageInfo packageInfo)
        {
            ...
            Debug.Log(packageInfo.errors);
            if (packageInfo.name == "com.unity.xr.legacyinputhelpers" && packageInfo.errors.Length==0)
            {
