...
namespace ambiens.archtoolkit.atexplore.actionSystem
{
    ...
    {
        ActionSequence currentSelectedSequence;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (Event.current != null) // Check event, only in the ongui method this can works
            {
                ...
                {
                    currentSelectedSequence = EditorGUIUtility.GetObjectPickerObject() as ActionSequence;
                    if (currentSelectedSequence != null)
                    {
                        if(!seqHolder.Sequences.Contains(currentSelectedSequence))
                            seqHolder.Sequences.Add(currentSelectedSequence);
                        currentSelectedSequence = null;
                    }
                }
            }
            ...
            if (GUILayout.Button(new GUIContent("NEW ACTION SEQUENCE", "Add a new Action Sequence to the scene")))
            {

                AddSequence(seqHolder);
            }
            if (GUILayout.Button(new GUIContent("+", "Add an existing Action Sequence")))
            {
                EditorGUIUtility.ShowObjectPicker<ActionSequence>(this.currentSelectedSequence, false, string.Empty, 222);
            }
