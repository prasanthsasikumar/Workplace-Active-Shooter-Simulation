...
namespace Archtoolkit.ATImport
{
    public class ProjectWindow : ProjectWindowBase
    {
        ...
        [MenuItem("Tools/Ambiens/ArchToolkit/AT+Sync Panel")]
        public static void Init()
        {

            Archtoolkit.ATImport.ProjectWindow.SetUpRenderPipeline();
            string defaultMaterialName = CMaterial.materialMaps[CMaterial.CurrentRenderPipeline][MaterialType.Standard];

            
            AvroImporter.firstImportSettings = new AvroImporterFirstImportSettings()
            {
                recalculateNormalAngle = true,
                m_generateLightmapUV = true,
                DefaultShaderName = defaultMaterialName
            };

            var window = (ProjectWindow)EditorWindow.GetWindow(typeof(ProjectWindow), false, "AT+Sync Panel");
        ...
        public static void SetUpRenderPipeline()
        {
            if (GraphicsSettings.renderPipelineAsset != null)
            {
                ...
                string rpname = GraphicsSettings.renderPipelineAsset.name.ToLower();
                ...
                else if (rpname.Contains("universal") || rpname.Contains("urp"))
                {
